...
Generated by `runfabric init` (worker, gcp-functions, ts).
...
npm run call:local -- --provider gcp-functions --host 127.0.0.1 --port 8787 --serve --watch
npm run call:local -- --provider gcp-functions --method GET --path /hello
npm run call:local -- --provider gcp-functions --event ./event.json
```
...
Set credentials for `gcp-functions` in your shell before running deploy:

```bash
export GCP_PROJECT_ID="your-value"
export GCP_SERVICE_ACCOUNT_KEY="your-value"
```
...
GCP_SERVICE_ACCOUNT_KEY=your-value
```
