...
Generated by `runfabric init` (cron, cloudflare-workers, ts).
...
npm run call:local -- --provider cloudflare-workers --host 127.0.0.1 --port 8787 --serve --watch
npm run call:local -- --provider cloudflare-workers --method GET --path /hello
npm run call:local -- --provider cloudflare-workers --event ./event.json
```
...
Set credentials for `cloudflare-workers` in your shell before running deploy:

```bash
export CLOUDFLARE_API_TOKEN="your-value"
export CLOUDFLARE_ACCOUNT_ID="your-value"
```
